...
left join {{ref('stg_orders')}} o on o.id = od.id
...
unique_source as (
    select *
            ,row_number() over(partition by customer_id,employee_id, purchase_order_id, inventory_id, supplier_id, creation_date) as row_number
    from source
)
